...
class Decoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.fuser = FeatureFusionBlock(1024+512, 512)
...
class MemoryReader(nn.Module):
    ...
    def get_affinity(self, mk, qk):
        B, CK, T, H, W = mk.shape
        mk = mk.flatten(start_dim=2)   #[B,K,THW]
        qk = qk.flatten(start_dim=2)   #[B,K,HW]  

        # See supplementary material
        a_sq = mk.pow(2).sum(1).unsqueeze(2)  #[B, THW, 1] [B, N, N]
        ab = mk.transpose(1, 2) @ qk   #[B, THW, HW]

        affinity = (2*ab-a_sq) / math.sqrt(CK)   # B, THW, NN
...
class STCN(nn.Module):
    def __init__(self, single_object):
        ...
        self.key_proj2 = KeyProjection(1024, keydim=64)
